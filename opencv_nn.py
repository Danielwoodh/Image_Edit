...
class imageClean():
	...
	def main(self, errors: list, filename: str, input_dir: str, output_dir: str, dim_x: int, dim_y: int, insensitivity: float):
		...
		mask_stack, x, y, w, h = self.contours(filename, img, img_ero, insensitivity)
	...
	def contours(self, filename, img, img_ero, insensitivity: float):
		...
		if x == 0 or y == 0 or w == img_ero.shape[1] or h == img_ero.shape[0]:
			...
			blob = cv2.dnn.blobFromImage(img, scalefactor=1.0, size=(img.shape[1], img.shape[0]), swapRB=False, crop=False)
			net.setInput(blob)
			out = net.forward()
			out = out[0, 0]
